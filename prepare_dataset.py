"""
extract data files and header files from corresponding .tar.gz files

since the header files in the WFDB_*.tar.gz files are out-of-date,
only the header files in the headers-only .tar.gz files will be used

TODO
----
add functionality of downloading the .tar.gz files from GCP using `download_blob`
"""
import os
import argparse
import tarfile
...
_url = "https://storage.cloud.google.com/physionetchallenge2021-public-datasets/"
_source_blob_name = "physionetchallenge2021-public-datasets"
...
def download_blob(bucket_name, source_blob_name, destination_file_name):
    ...
    # https://github.com/GoogleCloudPlatform/python-docs-samples/blob/HEAD/storage/cloud-client/storage_download_file.py

    try:
        from google.cloud import storage
    except Exception:
        raise ModuleNotFoundError("one should install the package \042google\042")
...
def run(
    ...
    _data_files = [
        ...
        for item in glob(os.path.join(_dir, "WFDB_*.tar.gz"))
        if os.path.basename(item) in _data_files
    ]
    _header_files = [
        os.path.basename(item)
        for item in glob(os.path.join(_dir, "*Headers.tar.gz"))
        if os.path.basename(item) in _header_files
    ]
    _output_directory = output_directory or input_directory
    assert all(
        [header_files[data_files.index(item)] in _header_files for item in _data_files]
            ...
            with tarfile.open(os.path.join(_dir, df), "r:gz") as tar:
                for member in tar.getmembers():
                    if member.isfile():
                        ...
                        if verbose:
                            print(
                                f"extracted '{os.path.join(_output_directory, df_name, member.name)}'"
                            )
