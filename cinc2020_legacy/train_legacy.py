"""
legacy model training file from CinC2020
...
def train(
    ...
    if config.lr_scheduler is None:
        ...
        raise NotImplementedError(
            "lr scheduler `{config.lr_scheduler.lower()}` not implemented for training"
        )
    ...
    for epoch in range(n_epochs):
        ...
        with tqdm(
            ...
            save_filename = (
                f"{save_prefix}{epoch + 1}_{get_date_str()}_{save_suffix}.pth"
            )
...
def evaluate(
    ...
    if debug:
        print(
            f"all_scalar_preds.shape = {all_scalar_preds.shape}, all_labels.shape = {all_labels.shape}"
        )
        head_num = 5
...
if __name__ == "__main__":
    ...
    try:
        ...
        torch.save(
            model.state_dict(), os.path.join(config.checkpoints, "INTERRUPTED.pth")
        )
