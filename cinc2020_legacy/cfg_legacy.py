...
PreprocCfg.rpeak_lead_num_thr = (
    8  # number of leads, used for merging rpeaks detected from 12 leads
)
...
FeatureCfg.pr_spike_inv_density_threshold = (
...
ModelCfg.tranche_model = ED(
    {
        "AB": os.path.join(
            _BASE_DIR, "saved_models", "ECG_CRNN_resnet_leadwise_none_tranche_AB.pth"
        ),
        "E": os.path.join(
            _BASE_DIR, "saved_models", "ECG_CRNN_resnet_leadwise_none_tranche_E.pth"
        ),
        "F": os.path.join(
            _BASE_DIR, "saved_models", "ECG_CRNN_resnet_leadwise_none_tranche_F.pth"
        ),
        # "all" refers to tranches A, B, E, F
        "all": os.path.join(
            _BASE_DIR, "saved_models", "ECG_CRNN_resnet_leadwise_none_tranche_all.pth"
        ),
    }
)
...
TrainCfg.tranche_classes = ED(
    {t: sorted(list(t_cw.keys())) for t, t_cw in TrainCfg.tranche_class_weights.items()}
)
...
TrainCfg.cnn_name = (
    "resnet_leadwise"  # "vgg16", "resnet", "vgg16_leadwise", "cpsc", "cpsc_leadwise"
)
