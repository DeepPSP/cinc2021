...
def find_challenge_files(label_directory, output_directory):
    ...
    for label_file in sorted(os.listdir(label_directory)):
        label_file_path = os.path.join(
            label_directory, label_file
        )  # Full path for label file
        if (
            os.path.isfile(label_file_path)
            and label_file.lower().endswith(".hea")
...
def load_classifier_outputs(output_files, classes):
    ...
    for i in range(num_recordings):
        ...
        recording_binary_outputs = [
            1
            if (
                (is_finite_number(entry) and float(entry) == 1)
                or (entry in ("True", "true", "T", "t"))
            )
            else 0
            for entry in recording_binary_outputs
        ]
        recording_scalar_outputs = [
            float(entry) if is_finite_number(entry) else 0
            for entry in recording_scalar_outputs
        ]
