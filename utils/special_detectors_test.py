...
def _test_sd(
    ...
    cc = special_detectors(
        data, fs, rpeak_fn="xqrs", leads=list(leads), verbose=verbose
    )
...
def get_parser() -> dict:
    ...
    parser.add_argument(
        ...
        "--full",
        action="store_true",
        help="run over the full dataset, overrides proportion",
        dest="full",
    )
    parser.add_argument(
        "-v",
...
if __name__ == "__main__":
    ...
    if _dr is None:
        raise ValueError("data directory could not be found!")
    if args.get("full", False):
        print("full dataset")
        all_candidates = _dr.df_stats[
            _dr.df_stats.diagnosis_scored.apply(lambda v: len(v)) != 0
        ].record.tolist()
    else:
        ...
        all_candidates = sorted(
            set(
                list_sum(
                    sample(
                        _dr.diagnoses_records_list[k],
                        int(round(len(_dr.diagnoses_records_list[k]) * size)),
                    )
                    for k in [
                        "Brady",
                        "STach",
