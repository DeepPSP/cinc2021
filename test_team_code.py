...
def test_load_model() -> NoReturn:
    ...
    for ld in lead_sets:
        try:
            model = load_model(MODEL_DIR, ld)
        except Exception:
            model = load_model(TrainCfg_ns.model_dir, ld)
        print(f"n_leads = {len(ld)}")
        print(model)
        print("#" * 80 + "\n\n")
...
def test_run_model() -> NoReturn:
    ...
    for ld in [
        ...
        try:
            test_model(
                MODEL_DIR,
                os.path.join(TEST_DATA_DIR, f"{ld}_leads"),
                os.path.join(TEST_OUTPUT_DIR, f"test_results_{ld}_leads"),
            )
        except Exception:
            test_model(
